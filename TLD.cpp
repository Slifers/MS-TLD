...
void TLD::init(const Mat& frame1, Mat& frame2,const Rect& box,FILE* bb_file){
  ...
  //Get Bounding Boxes
	...
	//init mean-shift tracker
	if (g_tracker != NULL)
		delete g_tracker;
	g_tracker = new ColorTracker();
	g_tracker->init(frame2, box.x, box.y, box.br().x, box.br().y);
...
void TLD::processFrame(const cv::Mat& img1,const cv::Mat& img2,Mat& frame,BoundingBox& bbnext,bool& lastboxfound, bool tl, FILE* bb_file){
  ...
  if(lastboxfound && tl){
	  ...
	  MStrack(img1, img2, frame,Rect(0,0,0,0));
  }
  else{
      tracked = false;
  }
      ...
      if(detected){                                               //   if Detected
          ...
          if (confident_detections==1){                                //if there is ONE such a cluster, re-initialize the tracker
			  ...
			  if (g_tracker != NULL)
				delete g_tracker;
			  g_tracker = new ColorTracker();
			  g_tracker->init(frame, bbnext.x, bbnext.y, bbnext.br().x, bbnext.br().y);
              ...
              if (close_detections>0){
                  bbnext.x = cvRound((float)(8*tbb.x+cx)/(float)(8+close_detections));   // weighted average trackers trajectory with the close detections
                  bbnext.y = cvRound((float)(8*tbb.y+cy)/(float)(8+close_detections));
                  bbnext.width = cvRound((float)(8*tbb.width+cw)/(float)(8+close_detections));
                  bbnext.height =  cvRound((float)(8*tbb.height+ch)/(float)(8+close_detections));
      ...
      if(detected){                           //  and detector is defined
          ...
          if (cconf.size()==1){
			...
			//	delete g_tracker;
			 // g_tracker = new ColorTracker();
			 // g_tracker->init(frame, bbnext.x, bbnext.y, bbnext.br().x, bbnext.br().y);
			  g_tracker->setLastBBox(bbnext.x, bbnext.y, bbnext.br().x, bbnext.br().y);
  ...
  lastbox=bbnext;
  if (lastboxfound)
    fprintf(bb_file,"%d,%d,%d,%d,%f\n",lastbox.x,lastbox.y,lastbox.width,lastbox.height,lastconf);
  else
...
void TLD::MStrack(const Mat& img1, const Mat& img2,Mat& frame,Rect box){
	BBox * bb0 = NULL;

	bb0 = g_tracker->track(frame);
	tbb.x=bb0->x;
	tbb.y=bb0->y;
	tbb.width=bb0->width;
	tbb.height=bb0->height;
	tracked = true;
...
void TLD::detect(const cv::Mat& frame){
  ...
  Rect sbb;
  sbb.x = tbb.x - tbb.width;
  sbb.y = tbb.y - tbb.height;
  sbb.width = 2 * tbb.width;
  sbb.height = 2 * tbb.height;

  for (int i=0;i<grid.size();i++){//FIXME: BottleNeck
	  if (sumframe<20 || nondetectframe<8==0  ||sbb.x<grid[i].x || sbb.y<grid[i].y || grid[i].br().x<sbb.br().x || grid[i].br().y<sbb.br().y) {
